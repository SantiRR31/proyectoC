...
def generar_informe_consolidado_egresos(mes_anio=None):
    ...
    try:
        ...
        sht.range("M5").value = config["no_cecati"]
        sht.range("R8").value = hoy.strftime("%B %Y").lower()
        ...
        carpeta_salida = os.path.join(config["carpeta_destino"], "Consolidado")
        os.makedirs(carpeta_salida, exist_ok=True)
        ...
        # Guardar archivo
        archivo_salida = os.path.join(config["carpeta_destino"], f"consolidado_egresos_{mes_actual}.xlsx")
        wb.save(archivo_salida)
